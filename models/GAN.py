...
def build_generator(gen_init_size, gen_upsample_flags, gen_c: List, gen_ks: List, gen_strides, gen_pads, z_dim, output_act=nn.Sigmoid):
    gen_layers = [norm(nn.Linear(z_dim, np.prod(gen_init_size)))]
    gen_layers.extend([nn.ReLU(), nn.BatchNorm1d(np.prod(gen_init_size), momentum=.9), View((1,) + gen_init_size, bs=True)])
    gen_c.insert(0, gen_c[-2])
    ...
    for usf, in_c, out_c, ks, stride, pad, i in \
        ...
        if i == len(gen_c) - 2:
            ...
            gen_layers.extend([nn.ReLU(), nn.BatchNorm2d(out_c, momentum=.9)])
